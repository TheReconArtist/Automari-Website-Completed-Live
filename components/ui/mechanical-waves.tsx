...
export function MechanicalWaves({
  ...
  useEffect(() => {
    ...
    requestAnimationFrame(() => {
      const { containerWidth, containerHeight } = updateSize()
      let currentWidth = containerWidth
      let currentHeight = containerHeight

      // Handle resize
      const handleResize = () => {
        const { containerWidth: w, containerHeight: h } = updateSize()
        currentWidth = w
        currentHeight = h
      }
      ...
      let phase = 0
      let isMoving = true
      let peak = { x: currentWidth / 2, y: currentHeight / 2 }
      ...
      const numNodes = Math.floor(currentWidth / NODE_SPACING_PX) + 1
      const numLines = Math.floor(currentHeight / LINE_SPACING_PX) + 1 + Math.ceil(PEAK_HEIGHT_PX / LINE_SPACING_PX)
      ...
      const step = () => {
        ...
        for (let lineIdx = 0; lineIdx < numLines; ++lineIdx) {
          ...
          ctx.lineTo(currentWidth, baseline)
          ctx.fill()
          ctx.stroke()
